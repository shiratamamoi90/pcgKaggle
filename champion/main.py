...
# Kaggle loads the agent via exec() with no __file__ defined, so guard it.
# Add both the agent dir and Kaggle's sandbox path so `import cg` resolves.
for _p in (_os.getcwd(), "/kaggle_simulations/agent"):
...
# Per-attack base damage (used to inject a damage/KO signal into the decoder so
# the policy doesn't have to re-derive "which action does more / secures a KO"
# from raw card-id embeddings -- the diagnosed ctx=0 representation gap).
attack_damage = {a.attackId: (a.damage or 0) for a in all_attack()}
...
# Reserved scalar feature slots appended after the vocab (engineered per-action
# signals carried via EmbeddingBag's per-index weights). [0]=attack dmg/300, [1]=KO flag.
decoder_extra_offset = decoder_size
decoder_size += 2

SEARCH_COUNT = int(_os.environ.get("NN_SEARCH", "1"))  # inference budget
...
def get_decoder_input(obs: Observation, actions: list[list[int]]) -> SparseVector:
    ...
    _def_hp = opp.active[0].hp if opp.active and opp.active[0] is not None else None
        ...
        for i in action:
            ...
            match o.type:
                ...
                case OptionType.ATTACK:
                    ...
                    dmg = attack_damage.get(o.attackId, 0)
                    if dmg:
                        sv.add(decoder_extra_offset + 0, dmg / 300.0)
                        if _def_hp is not None and dmg >= _def_hp:
                            sv.add(decoder_extra_offset + 1, 1.0)
...
# ----------------------------- inference wrapper -----------------------------
def _load_deck():
    c = _os.environ.get("DECK_PATH")
    if c and _os.path.exists(c):
        return [int(x) for x in open(c) if x.strip()]
    p = "deck.csv"
...
def _model_path():
    c = _os.environ.get("MODEL_PATH")
    if c and _os.path.exists(c):
        return c
    for p in ["model.pth", "/kaggle_simulations/agent/model.pth"]:
        if _os.path.exists(p):
            return p
    ms = sorted(glob.glob("models/model*.pth"))
...
if _mp:
    try:
        _model.load_state_dict(torch.load(_mp, map_location=_device))
    except Exception as _e:
        # Tolerate stale checkpoints whose decoder vocab predates the engineered
        # feature slots; the BC/self-play pipelines retrain a matching model.
        print(f"main_nn: skipped incompatible checkpoint {_mp}: {_e}")
_model.eval()
...
def agent(obs_dict):
    ...
    with torch.inference_mode():
        sel, _ = mcts_agent(obs_dict, _DECK, _model)
    return sel
